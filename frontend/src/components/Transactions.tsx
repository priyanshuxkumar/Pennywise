...
import { BadgeDollarSign, ChevronDown, Edit3, Trash2 } from 'lucide-react';
import { Button } from './Button';
import TransactionFilter from './DatePicker';
import { formatRelativeDate } from '../utils/lib';
import type { TransactionCardProp, TransactionDataProps } from '../utils/types';
import EntryEditor from './EntryEdit';
...
const Transactions = () => {
    ...
    const filteredData = selectedCategory === 'All' ? transactions : transactions.filter((item) => item.category === selectedCategory);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editTransaction, setEditTransaction] = useState<TransactionDataProps | null>(null);
    ...
    const handleEditModal = (id: string) => {
        ...
        setEditTransaction(transactions.find((item) => item.id === id) || null);
    };
    ...
    return (
        <>
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex flex-col gap-3">
                            {filteredData.map((item) => (
                                <Card key={item.id} {...item} onEdit={handleEditModal} onDelete={handleDelete} />
                            ))}
                        </div>
                    )}
                </div>
            ...
            {isEditModalOpen && (
                <EntryEditor
                    parsedInput={editTransaction as TransactionDataProps}
                    onSave={(data) => handleEdit(data as TransactionDataProps)}
        ...
        <div className="rounded-xl bg-white shadow-sm px-3 py-3 transition-colors duration-300 border border-black/5 dark:bg-neutral-900 dark:border-white/10">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3 min-w-0">
                    <div
                        className="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-2xl bg-gray-100 dark:bg-neutral-700"
                        aria-hidden="true"
                    >
                        <BadgeDollarSign />
                    </div>
                ...
                <div className="flex items-center justify-between md:justify-end gap-3">
                    <div className="font-bold text-sm text-black dark:text-white tabular-nums">
                        {type == 'expense' ? '-' : ''}
